<script setup>
    ...
    if(!userStore.biliUser) {
        ...
        if(qrKey.value) {
            checkQRCode().then(result => {
                if(!result || result.code !== 0) {getQRCode(); return}
                const code = result.data.code
                if(code === 86101 || code === 86090) checkInterval()
                else getQRCode()
            })
        } else {
            getQRCode()
  ...
  const getQRCode = () => {
    windowApi.getBiliRequestData({url: 'https://passport.bilibili.com/x/passport-login/web/qrcode/generate', option: {headers: headers}}).then(result => {
        qrKey.value = result.data.data.qrcode_key
        let opts = {
        ...
        QRCode.toDataURL(result.data.data.url, opts, (err, url) => {
            if(err) throw err
            qrcodeImg.value = url
        })
    })
  ...
  const checkQRCode = async () => {
    const result = await windowApi.getBiliRequestData({url: 'https://passport.bilibili.com/x/passport-login/web/qrcode/poll', option: {headers: headers, params: {qrcode_key: qrKey.value}}})
    return result.data
}
  const checkInterval = () => {
    checkQRTimer.value = setInterval(() => {
        checkQRCode().then(result => {
            if(!result || result.code !== 0) {getQRCode(); return}
            const code = result.data.code
            if(code === 86038) {getQRCode(); return}
            else if(code === 86101) return
            else if(code === 86090) return
            else if(code === 0) loginHandle(result.data)
        })
    }, 1000);
  }
  const loginHandle = async (data) => {
    ...
    // Store all cookies from redirect URL for biliFetch calls
    const rawQuery = data.url.split('?')[1]
    if (rawQuery) {
        const cookieParts = []
        rawQuery.split('&').forEach(pair => {
            ...
            if (key && !['gourl', 'csrf'].includes(key.toLowerCase())) {
                cookieParts.push(`${key}=${value}`)
                if (key === 'SESSDATA') localStorage.setItem('Sessdata', decodeURIComponent(value))
            }
        })
    ...
    const userInfo = await windowApi.getBiliRequestData({url: 'https://api.bilibili.com/x/web-interface/nav', option: {headers: headers}})
